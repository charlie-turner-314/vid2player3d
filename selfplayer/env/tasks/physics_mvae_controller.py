...
class PhysicsMVAEController:
    ...
    def create_sim(self):
        # TODO: replace with real beta from config somehow
        smpl_beta_dict = {}
        smpl_beta_dict["djokovic"] = [
            -0.9807,
            1.4050,
            -0.4144,
            1.4028,
        ...
        smpl_beta_dict["kyrgios"] = [
                        0.21906696259975433,
                        0.27422094345092773,
                        0.3669874668121338,
                        0.08729240298271179,
                        0.015572994947433472,
                        -0.16693685948848724,
                        -0.11255663633346558,
                        0.24428795278072357,
                        0.46144261956214905,
        ...
        if self.cfg_v2p.get("dual_mode") == "different":
            ...
            self.cfg_v2p["smpl_beta"] = [
                smpl_beta_dict[player[0]],
                smpl_beta_dict[player[1]],
            ]
            # let half of the envs play with the same beta, and the other half play with the different beta
            self.betas = (
                torch.FloatTensor(self.cfg_v2p["smpl_beta"])
                .repeat(self.num_envs // 2, 1)
                .to(self.device)
            )
        elif self.cfg_v2p.get("dual_mode") == "selfplay":
            player = self.cfg_v2p["player"]
            self.cfg_v2p["smpl_beta"] = [
                smpl_beta_dict[player],
                smpl_beta_dict[player],
            ]
            # same setup as above
            # TODO: Could this actually just be the same beta for all envs?
            self.betas = (
                torch.FloatTensor(self.cfg_v2p["smpl_beta"])
                .repeat(self.num_envs // 2, 1)
                .to(self.device)
    ...
    def reset(self, env_ids=None):
        if env_ids is None:
            if self._has_init and self.num_envs > 1:
                return
    ...
    def _reset_envs(self, env_ids):
        ...
        reset_reaction_env_ids = self._reset_reaction_buf.nonzero(
            as_tuple=False
        ).flatten()  # include reset_humanoid_env_ids
        reset_recovery_env_ids = self._reset_recovery_buf.nonzero(
            as_tuple=False
        ).flatten()
        reset_all_env_ids = (
            (self._reset_reaction_buf + self._reset_recovery_buf)
        ...
        if len(reset_humanoid_env_ids) > 0 or len(reset_reaction_env_ids) > 0:
            new_traj = self._physics_player.task.reset(
                reset_humanoid_env_ids, reset_reaction_env_ids
            )
    ...
    def _reset_reaction_tasks(self, env_ids, humanoid_env_ids=None):
        if self.cfg_v2p.get("use_history_ball_obs"):
            self._ball_obs[env_ids] = (
                self._ball_pos[env_ids]
                .view(-1, 1, 3)
                .repeat(1, self._obs_ball_traj_length, 1)
            )
        ...
        self._tar_time_total[env_ids] = self.cfg_v2p[
            "reset_reaction_nframes"
        ...
        if self.cfg_v2p.get("use_random_ball_target"):
            if self.cfg_v2p["use_random_ball_target"] == "continuous":
                ...
                self._target_bounce_pos[env_ids] = (
                    torch.rand((3,), device=self.device)
                    * (self._target_bounce_max - self._target_bounce_min)
                    + self._target_bounce_min
                )
            else:
                ...
                self._target_bounce_pos[env_ids[bounce_left]] = torch.FloatTensor(
                    [-3, 10, 0]
                ).to(self.device)
                self._target_bounce_pos[env_ids[bounce_middle]] = torch.FloatTensor(
                    [0, 10, 0]
                ).to(self.device)
                self._target_bounce_pos[env_ids[bounce_right]] = torch.FloatTensor(
                    [3, 10, 0]
                ).to(self.device)
    ...
    def _compute_reset(self):
        ...
        self._reset_recovery_buf = (self._tar_action == 1) & (
            has_contact | (self._ball_pos[:, 1] < self._root_pos[:, 1] - 1)
        )
        ...
        if self.cfg["env"].get("enableEarlyTermination"):
            ...
            terminate |= (self._reset_recovery_buf & ~has_contact) | (
                self._ball_pos[:, 1] < self._root_pos[:, 1] - 1
            )
